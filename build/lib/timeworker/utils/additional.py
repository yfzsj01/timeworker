...
import datetime
import calendar
from ..data.languages_info import stopwords
...
relative_time = {'the_day_before_yesterday_str': ['前天'],
                 ...
                 'current_day_str': {
                     '今天': '{}年{}月{}日'.format(*datetime.datetime.now().strftime("%Y-%m-%d").split('-'))}}


def str_preprocess(date_string):
    # stopwords
    for st_words in stopwords:
        date_string = date_string.replace(st_words, '')
...
def get_time_again(raw_time_str):
    ...
    if day_list:
        ...
        if len(weekday_list) == 1:
            ...
            y, w, _ = datetime.datetime.now().strftime("%Y-%W-%w").split('-')
            _, __, start_day = datetime.datetime.fromisocalendar(int(y), int(w), start_weekday).strftime(
                "%Y-%m-%d").split('-')
